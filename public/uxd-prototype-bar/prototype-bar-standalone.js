...
(function () {
  ...
  function hrefForScenario(scenario, id) {
    var fallback = '/provider/workspace?vision=model-fleet&nav=vision-model-fleet';
    var url;
    try {
      url = new URL((scenario && scenario.route) || fallback, window.location.origin);
    } catch (e) {
  ...
  function selectScenario(id) {
    var cfg = getConfig();
    var match = (cfg.scenarios || []).find(function (s) { return s.id === id; });
    var href = hrefForScenario(match, id);
