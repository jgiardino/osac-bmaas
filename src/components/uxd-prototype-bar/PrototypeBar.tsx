...
/** Build the URL for a scenario, including that scenario's page route (not just ?scenario=). */
function hrefForScenario(scenario: UxdBarScenario | undefined, id: string): string {
  const fallback = '/provider/workspace?vision=model-fleet&nav=vision-model-fleet';
  let url: URL;
  try {
    url = new URL(scenario?.route || fallback, window.location.origin);
  } catch {
...
export const PrototypeBar: React.FC = () => {
  ...
  const selectScenario = (id: string) => {
    ...
    const match = (cfg.scenarios || []).find((s) => s.id === id);
    const href = hrefForScenario(match, id);
    setActiveScenario(id);
    setStatus(`Scenario: ${match?.name || id}`);
    const here = `${window.location.pathname}${window.location.search}${window.location.hash}`;
    if (here === href) return;
    // Go to the scenario's page (AI Grid) plus ?scenario=. Do not only patch the
    // current URL — this workspace uses ?nav= for pages, so staying put leaves you
    // on Overview/Catalog and you have to click AI Grid again.
    navigate(href);
  };
